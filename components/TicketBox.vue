  ...
  <div class="box">
    ...
    <div class="box-body mt-2">
      ...
      <div class="d-flex gap-3 justify-content-between flex-wrap w-100">
        <h4>{{ data.acf.ticket_author }}</h4>
        <span>{{ data.acf.ticket_time }}</span>
      </div>
    </div>
    <p class="d-block">{{ data.acf.ticket_content }}</p>
    <div v-if="!noState" class="d-flex gap-3 mt-3">
      <div class="button" :class="btnStyle">
        <i class="bi bi-circle"/>{{ capitalizeFirstLetter(data.acf.ticket_status) }}
      </div>
      <div class="button">{{ capitalizeFirstLetter(data.acf.ticket_Priority) }}</div>
    </div>
  </div>
</template>
